...
const mrZierax = document.getElementById('mr-zierax');
let glitchInterval;

function glitchText() {
  mrZierax.style.setProperty('text-shadow', `0 0 2px ${randomColor()}, 0 0 5px ${randomColor()}`);
  mrZierax.style.setProperty('transform', `translate(-50%, -50%) rotate(${Math.random() * 5}deg)`);
}
...
glitchInterval = setInterval(glitchText, 50);

// Stop the glitch effect on hover (optional)
mrZierax.addEventListener('mouseover', () => clearInterval(glitchInterval));
mrZierax.addEventListener('mouseout', () => glitchInterval = setInterval(glitchText, 50));
